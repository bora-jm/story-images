...
export const GqlCustomHeaders = createParamDecorator(async (args: unknown, context: ExecutionContext) => {
  const ctx = GqlExecutionContext.create(context);
  const req = ctx.getContext().req;
  const header = new GqlCustomHeader(req);
  try {
...
export class GqlCustomHeader {
  ...
  constructor(req: Request) {
    ...
    this.timestamp = moment(req.get('x-timestamp')).isValid() ? moment(req.headers['x-timestamp']) : moment();
  }
}
