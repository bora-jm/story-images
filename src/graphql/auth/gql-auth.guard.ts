...
import { GqlServiceUser, InjectServiceUser } from '@lib/service-user';
...
export class GqlAuthGuard implements CanActivate {
  constructor(
    ...
    @InjectServiceUser()
    private readonly gqlUser: GqlServiceUser
  ) {}
  ...
  async checkUserScheme(scheme: string, token: string, roles: string[]) {
    ...
    const user = await this.gqlUser.getUserById({ id: decoded.id });
